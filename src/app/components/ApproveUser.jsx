




const Suspend = ({isVisible, onClose }) => {
  
    if ( !isVisible) return null;
  
   
  
    const handleClose = (e) => {
      if(e?.target.id === "wrapper") onClose();
    }
  
    
    return (
      <main onClick={handleClose}   id="wrapper" className="flex bg-black bg-opacity-25 inset-0 fixed backdrop-blur-sm min-h-screen items-center justify-center m-auto">
        <section  className="bg-white p-6 rounded w-[400px]">
    <div className="flex justify-between">
            <p className="text-sm:text-sm text-gray-600 font-sm">User request</p>
            <button onClick={()=>onClose()} className="text-gray-600 text-xl text-3x3">X</button>
          </div>
          <div><p className="text-sm mt-3 text-#1E233E text-center">
          ARE YOU CERTAIN YOU WANT TO APPROVE THE REQUEST
OF THIS USER TO BECOME A DRIVER ON THE PLATFORM.
ONCE APPROVED, THEY WILL BE NOTIFIED VIA EMAIL</p></div>
         
         <div className="flex justify-center mt-6">
         
                      <button onClick={()=>onClose()} className="text-[#1E233E] w-[120px] bg-white-500 text-black border-4 py-2 px-4 rounded-md mr-2">Cancel</button>
                    
                      <button className="text-white bg-[#576FF1]  py-2 px-4 rounded-md">Approve request</button>
                    
         </div> 
        </section>
      </main>
    );
  }
  
  export default Suspend;
  
  